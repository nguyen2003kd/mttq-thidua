...
export function DashboardContent({ title, description }: { title: string; description: string }) {
  ...
  const statusCounts = useMemo(() => {
    ...
    localities.forEach((loc) => {
      const record = table ? (scores[table.id]?.[loc.id] ?? emptyRecord) : emptyRecord;
      if (record.state === 'DA_CONG_BO') counts.PUBLISHED += 1;
      else if (record.state !== 'DRAFT') counts.IN_PROGRESS += 1;
      else counts.DRAFT += 1;
    });
    return counts;
  }, [localities, table, scores, emptyRecord]);
  ...
  const ranking = useMemo(() => {
    ...
    return localities
      .map((loc) => ({
        locality: loc,
        totalScore: (scores[table.id]?.[loc.id] ?? emptyRecord).totalScore,
      }))
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Điểm cao nhất" value={ranking[0]?.totalScore ?? 0} unit={`/${totalMax}`} icon={<TrendingUp className="h-5 w-5" />} />
      </div>
                ...
                return (
                  <div key={item.locality.id} className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      <span className="font-medium">{idx + 1}. {item.locality.name}</span>
                      <span className="tabular-nums text-muted-foreground">{item.totalScore} / {totalMax}</span>
                    </div>
                    <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
                      <div
                        className={cn('h-full rounded-full transition-all', idx === 0 ? 'bg-success' : 'bg-primary')}
                        style={{ width: `${percent}%` }}
                      />
                    </div>
                  </div>
