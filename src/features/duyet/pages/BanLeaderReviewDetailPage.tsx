...
import { ArrowLeft, Edit3, FileText, History, MessageSquareWarning, Paperclip, Save, Send, Sparkles } from 'lucide-react';
    ...
    <div className="space-y-1.5">
      ...
      <div className="space-y-1">
        {files.map((file) => {
          ...
          return <button key={file.id} type="button" onClick={(event) => { event.stopPropagation(); onPreview(file); }} className="flex max-w-full items-center gap-1 text-left text-sm text-primary hover:underline">
...
function ScoreReasonNotes({ result, draft }: { result?: SubmissionResultItem; draft?: LeaderScoreDraft }) {
  const specialistReason = result?.officialReason?.trim();
  const leaderReason = draft?.reason.trim();
  if (!specialistReason && !leaderReason) return null;

  return (
    <>
      {specialistReason && <p className="mt-2 border-t border-border pt-2"><span className="font-medium text-foreground">Lý do sửa điểm của chuyên viên:</span> {specialistReason}</p>}
      {leaderReason && <p className="mt-2 border-t border-border pt-2"><span className="font-medium text-foreground">Lý do Lãnh đạo chấm:</span> {leaderReason}</p>}
    </>
  );
}
...
export default function BanLeaderReviewDetailPage() {
  ...
  const forwardingFilesQuery = useQuery({
    ...
    queryFn: () => filesApi.list({
      ...
      entityId: submissionsQuery.data!.id,
  ...
  const uploadPendingLeaderAttachments = async () => {
    ...
    if (failedFiles.length > 0) {
      toast.warning(`Điểm đã được lưu nhưng ${failedFiles.length} tệp Lãnh đạo chưa tải lên được.`);
    }
  };
  ...
  return <div className="mx-auto flex min-h-full w-full max-w-[1480px] flex-col gap-5 pb-6">
    ...
    <section className="overflow-clip rounded-lg border border-border border-t-2 border-t-primary bg-card [&_a]:min-w-0 [&_a]:break-all [&_p]:break-words [&_table]:min-w-0 [&_table]:table-fixed [&_td]:min-w-0 [&_td]:whitespace-normal">
