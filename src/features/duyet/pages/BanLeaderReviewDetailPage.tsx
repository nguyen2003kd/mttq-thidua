...
import { ArrowLeft, FileText, History, MessageSquareWarning, Send } from 'lucide-react';
...
export default function BanLeaderReviewDetailPage() {
  ...
  return <div className="mx-auto flex min-h-full w-full max-w-[1480px] flex-col gap-5 pb-6">
    ...
    <section className="overflow-clip rounded-lg border border-border border-t-2 border-t-primary bg-card [&_a]:min-w-0 [&_a]:break-all [&_p]:break-words [&_table]:min-w-[1500px] [&_table]:table-fixed [&_td]:min-w-0 [&_td]:whitespace-normal">
      ...
      <div className="flex flex-wrap items-center justify-end gap-2 border-b border-border bg-card/95 px-4 py-3 sm:px-5"><Button variant="outline" disabled={!canProcess} onClick={() => setRevisionOpen(true)}><MessageSquareWarning className="mr-1.5 size-4" />Yêu cầu chỉnh sửa</Button><Button disabled={!canProcess} onClick={() => setApproveOpen(true)}><Send className="mr-1.5 size-4" />Duyệt &amp; trình Hội đồng</Button></div>
      <div className="overflow-x-auto"><Table className="min-w-[1120px] table-fixed"><colgroup><col className="w-[25%]" /><col className="w-[15%]" /><col className="w-[18%]" /><col className="w-[18%]" /><col className="w-[24%]" /></colgroup><TableHeader><TableRow className="bg-primary hover:bg-primary"><TableHead className="border-r border-white/30 bg-primary px-4 py-3 text-primary-foreground">Tiêu chí con</TableHead><TableHead className="border-r border-white/30 bg-primary px-4 py-3 text-primary-foreground">Minh chứng</TableHead><TableHead className="border-r border-white/30 bg-primary px-4 py-3 text-primary-foreground">Địa phương đề xuất</TableHead><TableHead className="border-r border-white/30 bg-primary px-4 py-3 text-primary-foreground">Chuyên viên chấm</TableHead><TableHead className="bg-primary px-4 py-3 text-primary-foreground">Nội dung diễn giải</TableHead></TableRow></TableHeader><TableBody>
        {resultItems.map(({ criterion, result }) => <TableRow key={criterion.id} className="align-top hover:bg-muted/60"><TableCell className="border-r border-primary/15 px-4 py-5"><p className="font-semibold leading-5">{criterion.content}</p></TableCell><TableCell className="border-r border-primary/15 px-4 py-5">{result?.files.length ? <div className="space-y-1">{result.files.map((file) => file.url ? <a key={file.id} href={file.url} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-sm text-primary hover:underline"><FileText className="size-4" />{file.originalName}</a> : <p key={file.id} className="flex items-center gap-1 text-sm text-muted-foreground"><FileText className="size-4" />{file.originalName}</p>)}</div> : <span className="text-xs text-muted-foreground">Chưa có minh chứng</span>}</TableCell><TableCell className="border-r border-primary/15 px-4 py-5"><div className="grid grid-cols-2 gap-2"><ScoreBox label="Điểm" value={result?.point ?? 0} maximum={criterion.maxPoint} /><ScoreBox label="Điểm thưởng" value={result?.bonusPoint ?? 0} maximum={criterion.maxBonusPoint} /></div></TableCell><TableCell className="border-r border-primary/15 px-4 py-5"><div className="grid grid-cols-2 gap-2"><ScoreBox label="Điểm" value={result?.officialPoint ?? result?.point ?? 0} maximum={criterion.maxPoint} /><ScoreBox label="Điểm thưởng" value={result?.officialBonusPoint ?? result?.bonusPoint ?? 0} maximum={criterion.maxBonusPoint} /></div></TableCell><TableCell className="px-4 py-5 text-sm leading-6 text-muted-foreground">{result?.explanation || '—'}{result?.officialReason && <p className="mt-2 border-t border-border pt-2"><span className="font-medium text-foreground">Lý do chấm:</span> {result.officialReason}</p>}</TableCell></TableRow>)}
