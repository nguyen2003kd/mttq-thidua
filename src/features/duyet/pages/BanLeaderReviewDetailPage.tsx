...
function LeaderCriterionDetailDialog({
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[calc(100dvh-2rem)] max-w-2xl overflow-y-auto p-0 sm:max-w-2xl">
      ...
      <DialogFooter className="border-t border-border px-6 py-4"><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Đóng</Button>{criterion.type !== 'Supplementary' && <Button type="button" disabled={editDisabled} disabledReason={editDisabled ? 'Hồ sơ đã chuyển bước nên không thể cập nhật điểm.' : undefined} onClick={onEdit}><Edit3 className="size-4" />Sửa điểm</Button>}</DialogFooter>
    </DialogContent>
  </Dialog>;
}
