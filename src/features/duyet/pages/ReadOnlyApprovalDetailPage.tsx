...
import { specialistApi, type SubmissionApi, type SubmissionResultFile, type SubmissionResultItem, type SubmissionStage } from '@/features/cham-diem/api/specialistApi';
...
function ForwardingFilesSection({ specialistFiles, leaderFiles, onPreview }: { specialistFiles: PreviewableFile[]; leaderFiles: PreviewableFile[]; onPreview: (file: PreviewableFile) => void }) {
  const groups = [
    { label: 'Chuyên viên gửi Lãnh đạo ban', files: specialistFiles },
    { label: 'Lãnh đạo ban gửi Hội đồng', files: leaderFiles },
  ].filter((group) => group.files.length > 0);
  const total = groups.reduce((count, group) => count + group.files.length, 0);
  if (total === 0) return null;

  return <section className="rounded-lg border border-primary/20 border-l-4 border-l-primary bg-primary/[0.04] p-4" aria-label="Tệp đính kèm hồ sơ"><div className="flex flex-wrap items-center gap-2"><Paperclip className="size-4 text-primary" /><h2 className="text-sm font-bold text-primary">Tệp đính kèm hồ sơ</h2><span className="rounded-full bg-primary/10 px-2 py-0.5 text-xs font-semibold text-primary">{total} tệp</span></div><div className="mt-3 grid gap-3 md:grid-cols-2">{groups.map((group) => <div key={group.label} className="min-w-0 rounded-md border border-primary/15 bg-card/80 p-3"><p className="text-xs font-semibold text-primary">{group.label}</p><div className="mt-2 flex flex-wrap gap-2">{group.files.map((file) => <button key={file.id} type="button" onClick={() => onPreview(file)} className="flex max-w-full min-w-0 items-center gap-2 rounded-md border border-primary/20 bg-card px-3 py-2 text-left text-sm text-primary transition-colors hover:bg-primary/10"><FileText className="size-4 shrink-0" /><span className="min-w-0 break-all">{file.displayName || file.originalName}</span></button>)}</div></div>)}</div></section>;
}
...
export default function ReadOnlyApprovalDetailPage({ reviewer }: { reviewer: Reviewer }) {
  ...
  const forwardingFilesQuery = useQuery({
    queryKey: ['approval-detail-forwarding-files', reviewer, submissionsQuery.data?.id],
    ...
    enabled: Boolean(submissionsQuery.data?.id && reviewer === 'council'),
  });
  const specialistForwardingFilesQuery = useQuery({
    queryKey: ['approval-detail-specialist-forwarding-files', reviewer, submissionsQuery.data?.id],
  ...
  const specialistForwardingFiles = specialistForwardingFilesQuery.data?.items ?? [];
  const forwardingFiles = forwardingFilesQuery.data?.items ?? [];
  ...
  return <div className="mx-auto flex w-full max-w-[1480px] flex-col gap-5 pb-6">
    ...
    <ForwardingFilesSection specialistFiles={specialistForwardingFiles} leaderFiles={forwardingFiles} onPreview={setPreviewFile} />
