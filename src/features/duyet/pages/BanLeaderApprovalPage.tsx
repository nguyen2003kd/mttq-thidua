...
export default function BanLeaderApprovalPage() {
  ...
  const handleApprove = (row: ApprovalRow) => {
    ...
    toast.success('Đã duyệt', { description: `${row.locality.name} chuyển sang chờ Hội đồng TĐKT.` });
  };

  const handleReject = () => {
    if (!user || !rejectRow || !reason.trim()) return;
    reject(rejectRow.table.id, rejectRow.locality.id, reason, user.name, user.role);
    toast.success('Đã trả lại', { description: `${rejectRow.locality.name} đã được trả về nháp.` });
    setRejectRow(null);
    setReason('');
  };
  ...
  const columns = useMemo<ColumnDef<ApprovalRow>[]>(
    () => [
      ...
      {
        accessorFn: (row) => row.record.totalScore,
        header: 'Tổng điểm',
        meta: { align: 'center' },
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center justify-end gap-2">
            ...
            <Button size="sm" variant="outline" className="text-success hover:bg-success/10" action="approve" state="CHO_DUYET_BAN" onClick={() => handleApprove(row.original)}>
              <Check className="h-3.5 w-3.5 mr-1.5" />
              Duyệt
            </Button>
            <Button size="sm" variant="outline" className="text-destructive hover:bg-destructive/10" action="reject" state="CHO_DUYET_BAN" onClick={() => setRejectRow(row.original)}>
    ...
    <div className="space-y-6">
      ...
      {rows.length === 0 ? (
        <EmptyState
          ...
          description="Hiện chưa có địa phương nào nộp bảng điểm để Ban xem xét."
      ...
      <Dialog open={!!rejectRow} onOpenChange={(v) => { if (!v) { setRejectRow(null); setReason(''); } }}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" action="reject" state="CHO_DUYET_BAN" onClick={handleReject} disabled={!reason.trim()}>Trả lại</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
