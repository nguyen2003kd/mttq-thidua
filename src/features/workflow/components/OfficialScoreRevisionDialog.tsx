...
/** Xem điểm Chuyên viên đã sửa và tệp category=score-update của đúng tiêu chí con. */
export function OfficialScoreRevisionDialog({ open, onOpenChange, result, criterionLabel }: OfficialScoreRevisionDialogProps) {
  const filesQuery = useQuery({
    ...
    queryFn: () => filesApi.list({ entityType: 'SubmissionResult', entityId: result!.id, category: 'score-update', page: 1, pageSize: 100 }),
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[calc(100dvh-2rem)] max-w-2xl overflow-y-auto sm:max-w-2xl">
      <DialogHeader><DialogTitle>Điểm chuyên viên đã sửa</DialogTitle><DialogDescription>Xem điểm, lý do và tệp đính kèm của lần điều chỉnh cho tiêu chí này.</DialogDescription></DialogHeader>
